...
% \VignetteDepends{robustbase,cellWise,VariableScreening,R.matlab,doParallel,ggpubr}
...
computational burden. If data-driven random projections are employed, only the
...
assumes that model employs a linear predictor, i.e.,~a linear combination of
...
p2 <- plot(spar_res, plot_type = "Val_numAct") +
  theme(axis.text.x = element_text(angle = 45,vjust = 0.5))
p4 <- plot(spar_res, plot_type = "res-vs-fitted", xfit = example_data$x,
     yfit = example_data$y)
p3 <- plot(spar_res,  plot_type = "coefs")
p <- ggpubr::ggarrange(p1, p2, p3, p4,
       ncol = 4, nrow = 1)
ggsave(filename = "Figures/plot_example.pdf", p,
       width = 14, height = 4.5)
@
\begin{figure}[t!]
\centering
\includegraphics[width=1\textwidth]{Figures/plot_example.pdf}
\caption{\code{plot} methods for \class{spar} object. The red dots in the first two figures show the best $\nu$ chosen on the validation set for the optimal number of models $M=5$.
\label{fig:plot_example}}
\end{figure}
%
For class `\code{spar.cv}' there is the extra argument
\code{opt_par = c("best", "1se")} which is only used for \code{plot_type = "res-vs-fitted"} and indicates whether the predictions
