...
coefficient of the GLM containing only $\boldsymbol X_j$ as a predictor:
...
correlation of a tilted version of $\boldsymbol X_j$ with $\boldsymbol y$ where the effect of other
...
in $\boldsymbol X_j$ of the variance in the response of the conditional cumulative
distribution function $\Prob(\boldsymbol X_j\leq x|\boldsymbol y)$ \citep{cui2015model}.
...
{\boldsymbol\gamma}^k$, $\hat {\boldsymbol\beta}_{\bar I_k}^k=0$, where $\bar I_k$ contains the index of the variables not
contained in $I_k$.
\end{enumerate}
...
coefficient of $\boldsymbol X_j$ for $j =1,\dots,p$  in a univariate GLM using the
...
correlation between $\boldsymbol y$ and $\boldsymbol X_j$ using the function \fct{stats::cor}. It
...
function of package \pkg{doParallel} \citep{pkg:doParallel}. Our experiments suggest that parallelization
...
x[id_outliers_x] <- x[id_outliers_x] + 100
@
...
vector, making it a suitable method for high-dimensional settings with correlated predictors where the sparsity of the problem is unknown.
