...
\cite{Dunson2020TargRandProj} employ the algorithm with the sparse random
...
predictions and variable ranking in settings exhibiting different degrees of
sparsity in the coefficients.

With numerous methods available for generating screening coefficients and
random projections -- each with distinct advantages across different data
settings -- \pkg{spareg} offers a diverse selection of screening coefficients
and multiple procedures for constructing random projection matrices. Designed
for flexibility, the package provides a versatile framework that allows users
to extend its screening and projection techniques with custom procedures. This
is achieved through \proglang{R}'s \proglang{S}3 classes, ensuring a seamless
...
quadratic discriminant analysis (more details on this procedure will follow
in Section~\ref{sec-ext-rp}).
...
$x_{ij}$ is the $j$-th entry of the vector $x_i$.
...
propose employing the ridge estimator when the penalty term converges to zero
while \cite{cho2012high} propose using the tilted correlation, i.e.,~the
correlation of a tilted version of $X_j$ with $y$ where the effect of other
variables is reduced. For discrete outcomes, joint feature screening
...
in $X_j$ of the variance in the response of the conditional cumulative
...
across ensemble models. Instead of relying on a fixed threshold or number of
predictors to be screened,  predictors are sampled with probabilities
proportional to their screening score
...
$m\geq m_0=\mathcal O(\epsilon^{-2}\log(n))$.
...
$\Phi_{ij} =  \pm 1/\sqrt{\psi}$ with probability $\psi/2$ and zero otherwise for
$\quad 0<\psi\leq 1$,
...
proposed by \citep{LiHastie2006VerySparseRP} for preserving approximate $\ell_1$
...
and the vector of responses $y$ for the  Gaussian  family by subtracting the
sample column mean and dividing by the sample standard deviation.
...
$\hat \beta = \sum_{k=1}^M\hat \beta^k / M$ (this results in averaging of the
models on the link level) or via the fitted values by taking a
\emph{simple average} of $\hat y = \sum_{k=1}^Mg^{-1}(\boldsymbol x_i^\top\hat\beta^k) / M$.
...
@
%
In this section we rely for illustration purposes on the simulated example data
...
from a linear regression model. Usign the default values, data is generated
...
\begin{itemize}
\item  \code{x}  an $n \times p$ numeric matrix of predictor variables where
$n < p$,
...
model employed for each element of the ensemble.
%
...
absolute values of the estimated coefficients from the marginal models, chosen
...
a logical indicating whether the function should use it.
...
column index-vectors corresponding to variables that should be kept after
...
However, it is possible to specify whether the data associated with the random
projection (relevant for data-driven random projections) should be updated in
each fold iteration with the corresponding training data. This is achieved by
modifying elements of the \class{randomprojection} object, which we will
exemplify in Section~\ref{sec-extensibility}.
%
%
%
...
\code{...} (to be saved as attributes) and  \code{control} (a list of
...
coefficient of $x_j$ for $j =1,\dots,p$  in a univariate GLM using the
...
\fct{stats::glm} such as weights, family, offsets.
%
\item  \code{screen_cor()} -- computes the screening coefficients by the
correlation between $y$ and $x_j$ using the function \fct{stats::cor}. It
...
\in\mathbb{R}}\sum_{i=1}^n -\ell(\beta;y_i,x_i) + \frac{\varepsilon}{2}
\sum_{j=1}^p{\beta}_j^2, \, \varepsilon > 0
$$
The function
relies on \code{glmnet::glmnet()} and, while it assumes by default
...
screening coefficient. This function should have as following arguments:
...
models in the SPAR algorithm; defaults to \code{NULL}. In this case the
whole data will be used for estimating the
...
will be the one in \citep{ACHLIOPTAS2003JL}. The value of $\psi$ can be passed
through the \code{...} argument and defaults to  \code{psi = 1}.
%
\item \code{rp_cw()} -- sparse embedding random projection in
\cite{Clarkson2013LowRankApprox}. If specified as \code{rp_cw(data = TRUE)},
...
object, \code{m}, the target dimension and a vector of indices \code{included_vector}
...
fact that screening can be employed pre-projection. It can return a matrix or
...
\cite{parzer2024glms} these would be the ridge coefficients estimated on the
whole data set) need to be computed only once and not every time
...
If specified by the user, this function should take as argument only \code{...}.
...
The functions \code{update_data_fun()} and \code{update_rpm_w_data()} are
\code{NULL} as this conventional random projection is data-agnostic.
%
...
of the SPAR algorithm. In the case of function \code{spar_glmnet()} this
...
\code{c("1se","best")} and chooses whether to select the best pair of \code{nus}
...
are computed as $y- \widehat y$, where $\widehat y$ is the prediction computed
on response level.
...
smallest across the marginal models and then represented in the plot.
\end{itemize}
\item \code{plot_along} -- one of \code{c("nu","nummod")}; for
\code{plot_type = "Val_Measure"} or \code{"Val_numAct"} it indicates whether the
...
function of package \pkg{doParallel} \citep{pkg:doParallel}. In general, the parallelization seems to
pay off especially for data sets with a larger number of observations $n$. A minimal example with a correlation-based
...
spar_example <- spar(example_data$x, example_data$y,
  ...
  rp = rp_sparse(psi = 1/ncol(example_data$x)), measure = "mse")
spar_example
@
%
...
information related to the family. Given that we will rely on
\code{glmnet::glmnet()} in each loop, the family object is modified to a
...
algorithms). This modified version is
...
generating  the random projection, $B_2=50$ and $\xi=0.25$.
<<>>=
...
contaminate $25$\% of the \code{example\_data} with outliers in the predictors
and consider a binary version of the response variable \code{y}.
<<contaminate_x_y>>=
perc_cont <- 0.25
x <- example_data2$x;
set.seed(123)
...
To improve stability in the estimates, we set the upper bound on goal dimension
of the random projection to 25 (default would be $n/2=100$).
<<compare_glmrob_glm>>=
set.seed(1234)
...
\ifPositive
<<echo=FALSE>>=
if (is_paper && !dir.exists("Figures")) dir.create("Figures")
@
...
the 1-standard-error rule (bottom) for the \emph{Isomap faces} data set.
...
<<plotobsvspred, echo=FALSE, include=FALSE>>=
if (is_paper) {
  i <- 3
...
spar_darwin <- spar.cv(darwin$x, darwin$y,
  family = binomial(logit), screencoef = screen_glmnet(),
  nummods = c(5, 10, 20, 50), measure = "1-auc")
spar_darwin
@
...
<<plotdarwincoefreal,echo=FALSE, include=FALSE>>=
if (is_paper) {
ntasks <- 25
nfeat <- 18
reorder_ind <- c(outer(
...
\end{figure}
In general we observe that the different features measures across different
tasks have the same impact on the probability of AD (observable by the blocks
of blue or red lines).

<<clean_up, include=FALSE, echo=FALSE>>=
# clean up
if (clean_up) {
  if (file.exists("darwin.zip")) invisible(file.remove("darwin.zip"))
  if (file.exists("data.csv")) invisible(file.remove("data.csv"))
  if (file.exists("face_data.mat")) invisible(file.remove("face_data.mat"))
  if (file.exists("cannings_example.rda")) invisible(file.remove("cannings_example.rda"))
  if (file.exists("faces_res.rda")) invisible(file.remove("faces_res.rda"))
  if (file.exists("darwin_res.rda")) invisible(file.remove("darwin_res.rda"))
  if (dir.exists("Figures")) {
    invisible(file.remove("Figures/*"))
  }
}
@
\fi
...
Package \pkg{spareg} can be employed for modeling data in a high-dimensional
setting, where the number of predictors is much higher than the number of
observations. The package provides an implementation an algorithm for  sparse
projected and average regression (SPAR) proposed in \cite{parzer2024glms}
...
for linear
regression and extended to GLMs in \cite{parzer2024glms}.  This method has the
advantage of approximately capturing the true $\beta$
in the span of the random projection matrix, i.e.,~it ensures that the true
