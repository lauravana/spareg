...
GLM estimation on the projected predictors (both classical and penalized)
 can be done using
\pkg{sklearn.linear\_model} module.
...
ensembles of
machine learning models (including GLMs) via stacking, boosting or bagging \proglang{Stata}, but random subspace methods or similar approaches to
dimensionality reduction are not implemented at the time of writing.
...
We use the default of $M=20$ models in the ensemble.
