...
print(coef(spar_res), show = 10)
coef(spar_res, aggregate = "none")
coef(spar_res, nummod = 30, aggregate = "none")
summary(coef(spar_res, aggregate = "none"))
summary(coef(spar_res, aggregate = "median"))
summary(coef(spar_res,  nummod = 30,aggregate = "median"))
get_intercept(coef(spar_res))
get_intercept(coef(spar_res, aggregate = "none"))
get_intercept(coef(spar_res, aggregate = "median"))
coef(spar_cv, aggregate = "none")
coef(spar_cv, opt_par = "1se", aggregate = "none")
coef(spar_cv, opt_par = "best", aggregate = "none")
coef(spar_cv, nummod = 20, nu = 0.0049,
     opt_par = "best", aggregate = "none")
...
value with minimal  \code{measure} is used if not provided.
%
\item \code{coefs} --	optional vector of coefficients to be used directly in
the prediction.
\end{itemize}
...
\code{"val_numactive"};  if \code{plot_type = "res-vs-fitted"}, it is used
...
We can now compare the two approaches by directly looking at the validation \code{measure} value achieved on the
...
@
The predictive performance on the test data is given by:
