...
in $\boldsymbol X_{j}$ of the variance in the response of the conditional cumulative
distribution function $\Prob(\boldsymbol X_{j}\leq x|\boldsymbol y)$ \citep{cui2015model}.
