...
@
%
In this section we rely for illustration purposes on the simulated example data set which contains $n=200$ observations of a continuous response
...
number of models \code{nummod} fixed or viceversa. This argument is ignored when \code{plot_type = "res-vs-fitted"} or \code{plot_type = "coefs"}.
...
\code{plot_type = "coefs"} which gives the limits of the predictors' plot range; defaults to \code{c(1, p)}.
...
if (!file.exists("face_data.mat")) {
  ...
  download.file(url, "face_data.mat.Z")
  if(os == "windows"){
    Decompress7Zip(zipFileName =  "face_data.mat.Z", outputDirectory=getwd(), TRUE)
  } else {
    uncompress_linux(filename =  "face_data.mat.Z")
  }
