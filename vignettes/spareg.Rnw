...
the computational challenges of large predictor sets. The package makes
...
correlation of a tilted version of $X_j$ with $\boldsymbol y$ where the effect of other
...
distribution function $\Prob(X_j\leq x|y)$ \citep{cui2015model}.
...
across ensemble models. Instead of relying on a fixed threshold $\delta$, predictors are sampled with probabilities
...
\emph{simple average} of $\hat y = \sum_{k=1}^Mg^{-1}(\boldsymbol x_i^\top\hat{\boldsymbol\beta}^k) / M$.
...
\fct{stats::glm} (e.g.,~such as \code{weights}, \code{family}, \code{offset}).
%
\item  \code{screen_cor()} -- computes the screening coefficients by the
correlation between $\boldsymbol y$ and $X_j$ using the function \fct{stats::cor}. It
...
controls through the \code{control} argument to \code{glmnet::glmnet()} (e.g.,~such
as \code{alpha = 1}).
...
the whole data will be used for estimating the
...
constructed as $\Phi=BD\in \mathbb{R}^{m\times p}$, where $B$ is a $(p\times p)$  binary matrix, where for each column $j$ an index is uniformly sampled from $\{1,\ldots,m\}$ and the corresponding entry is set to one, and $D$ is a $(p\times p)$ diagonal matrix,
...
The function \code{update_fun()} only converts the \class{family} object to a string and adds it as an attribute. Function \code{update_rpm_w_data()} is
...
further argument which is the object of class `\code{sparmodel}' itself. It returns a list of two elements \code{list(gammas = , intercept = )}, which contains the vector of coefficients (\code{gammas}) and the value of the \code{intercept}.
%
...
of the SPAR algorithm. This function returns the `\code{sparmodel}' object after modifying it.  In the case of function \code{spar_glmnet()} this
...
using function the \code{screenIID()} in \pkg{VariableScreening}.
<<>>=
...
contaminate $25$\% of \code{example\_data2} above with outliers in the predictors
...
if (is_paper) {
  ...
  ggsave(filename =
           sprintf("Figures/faces_obs_%i.pdf",
                   i), p, width = 4.5,
         height = 4.5)
}
@
...
plot(spar_faces)
@
If we wish do visualize the results for $M=5$, which is the value chosen by the 1-standard-error rule, we can specify this through the \code{nummod} argument (see Figure~\ref{fig:facesplot_valmeasure} bottom panel).
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.45\textwidth]{Figures/faces_obs_179.pdf}
...
Moreover, the flexibility and adaptability of the \pkg{spareg} package in dealing with different types of screening, random projection and types of GLMs, make it an
